...
router.get('/getUserLoginLogs',function(req,res,next){
	...
	var _total = 0;
	LoginLogModel.find({userId:_userId}).skip(_pageIndex*_pageSize).limit(_pageSize).sort({logoutTime:-1}).exec(function(err,data){
		if(err){
			res.sendStatus(404);
			res.end();
			return;
		}
		LoginLogModel.find({userId:_userId},function(rerr,rdata){
			if(rerr){
				res.sendStatus(404);
				res.end();
				return;
...
router.get('/delUserLoginLog',function(req,res,next){
	...
	if((req.get('authSecret') && req.get('authSecret') === globalConfig.authSecret) || (req.session.isLogin && req.session.user.userType>0)){
		...
		LoginLogModel.findById(log_id,function(ferr,fdoc){
			...
			//fdoc -> Null || Entity
			if(ferr){
				res.json({retCode:100020,msg:'无此条登录日志',data:null});
				res.end();
				return;
			}

			LoginLogModel.remove({_id:log_id},function(rerr,rdoc){
				//ferr -> Object || Null
				//fdoc -> Null || Entit
				if(rerr){
					res.sendStatus(500);
				...
				res.json({retCode:0,msg:'查询成功',data:rdoc});
				res.end();
			});
		});
	}else{
...
router.post('/addAdmin',function(req,res,next){
	if((req.get('authSecret') && req.get('authSecret') === globalConfig.authSecret) || (req.session.isLogin && req.session.user.userType>0)){
		...
		UserModel.findOne({account:_account},function(ferr,fdoc){
			...
			UserModel.update({ _id:fdoc._id },{ userType:1, setAdminTime:Date.now() },function(uerr,udoc){
				if(uerr){
					res.end(500);
					res.end();
					return;
				...
				res.json({retCode:0,msg:'添加成功',data:null});
				res.end();
			})
		});
	}else{
...
router.get('/delAdmin',function(req,res,next){
	...
	if((req.get('authSecret') && req.get('authSecret') === globalConfig.authSecret) || (req.session.isLogin && req.session.user.userType>0)){
		...
		UserModel.findById(user_id,function(err,doc){
			...
			//检测是否是管理员
			if(doc.userType < 1){
				res.json({retCode:100023,msg:'该用户不是管理员',data:null});
				res.end();
				return;
			}
			UserModel.update({_id:user_id},{ userType:0 },function(rerr,rdoc){
				if(rerr){
					res.sendStatus(500);
					res.end();
					return;
				}
				res.json({retCode:0,msg:'删除管理员成功',data:null});
				res.end();
			})
...
router.get('/getWebPreview',function(req,res,next){
	if((req.get('authSecret') && req.get('authSecret') === globalConfig.authSecret) || (req.session.isLogin && req.session.user.userType>0)){
		UserModel.count(function(caerr,cacol){
			var _totalMember = 0;
			var _yesRegMember = 0;
			var _yesLogMember = 0;
